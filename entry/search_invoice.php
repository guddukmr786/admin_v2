...
if (strlen($search_string) >= 1 && $search_string !== ' ') {
	...
	$query = 'SELECT * FROM invoice_details WHERE CONCAT(name,"|",email) LIKE "%'.$search_string.'%" AND hotel_id = "'.$hotel_id.'"';
	...
	if (!empty($result_array)) {
		...
		foreach ($result_array as $result) {
			...
			$html .= '<td class="name">gstString</td>';
			...
			$o = str_replace('gstString', $t_gstin, $o);
